...
The one request asks independent typed questions for route, urgency, immediate notification, reply requirement, and bulk/subscription status. Code—not Jev—owns policy and side effects. A durable claim is written before the paid request, so concurrent workers and crash retries do not spend twice. Malformed, uncertain, unavailable, or unparseable cases become local `review` decisions and leave Inbox unchanged. `engine decisions` lists every current decision with its route, urgency, action state, confidence, and closed error code without fetching message content. Claims left `processing` for ten minutes are parked once as `decision_interrupted` for human review without another request.

`engine recover <uid>` immediately releases an abandoned current-epoch processing claim into review without making a Jev call. The `--retry-jev --confirm-new-jev-call` path is narrower: it is accepted only for `openrouter_api_key_missing`, which proves the original request never left Envelope. It fetches and reclassifies that exact current-UIDVALIDITY message and never rewinds the new-mail watermark. Ambiguous network/model failures cannot be retried through this path.
