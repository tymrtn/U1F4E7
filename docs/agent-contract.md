...
**Relationship facts.** `known_contact`, `frequent_contact`, `cold_email`, `unknown_domain` and `reply_to_thread` come from local evidence that inbound mail can't create. Outbound history means messages cached from the account's detected Sent folder; a From equal to the account address anywhere else (a spoofed message in INBOX, for example) doesn't count. A contact makes a recipient known only when a person curated it with CLI `contacts add` or `tag`. Contacts written over MCP (`add`, `tag`, `untag`) or by `contacts import` are kept in the address book and never vouch. `cold_email` fires when any recipient lacks that history, and `unknown_domain` when any recipient's domain is unseen, so one new address among known ones still carries its first-contact signal. `reply_to_thread` needs the `In-Reply-To` to resolve to exactly one cached thread in the account, with every recipient on a Sent-folder message in that thread. An unresolved or forged parent, a reply to a thread only the other side wrote in, or a newly added recipient all score as a fresh compose. A bounded lookup that runs out of rows leaves a fact unknown and asserts nothing.
