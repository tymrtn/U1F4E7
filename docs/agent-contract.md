...
For the sweep specifically: The claimed row is reloaded as the authoritative snapshot for final Governor attribution and SMTP; every content/recipient/attachment/metadata/status/schedule mutation primitive carries an editable-status predicate **inside its UPDATE statement**, so a `sending` (or `sent`/`discarded`) row is atomically immutable — even an interleaving where the claim lands between a caller's pre-read and its write cannot mutate the snapshot between reload and SMTP. Because the due query selects only `status='draft'`, a crash or later local DB failure leaves the row `sending` (not editable, never re-sent) until recovery resolves it (see **Crash safety** below); it is never returned to due while its transmission may have started. Pre-SMTP failures release the claim by reason: durable Governor `review`/`deny` verdicts park it as `pending_review` (no per-sweep retry storm), while transient failures (Governor unavailable, credentials, SMTP connection) release it back to `draft` for a later sweep. The sweep also loads the bot's declaration persisted at queue time (draft metadata `attribution` key, revision-bound) and resolves `declared ∪ derived` before scoring; a bot-originated draft with no valid current declaration fails closed here (host facts never substitute) and is retried a bounded number of times (**3**) before parking as `pending_review` with `send_after` cleared and `park_reason: attribution_exhausted` — a material draft revision invalidates the declaration and resets that counter. After SMTP acceptance the claim is only ever left via the sent state — a transmitted draft is never returned to due. After SMTP acceptance **and** durable local sent-state persistence, every send surface (sweep, CLI `draft send`, MCP `send_draft` — shared `draft_cleanup` primitives) removes the now-stale provider Drafts copy — identity-safe and fail-closed: the folder must come from the detected-folder cache (e.g. Gmail's `[Gmail]/Drafts`; a cache miss or read error skips cleanup, there is no guessed fallback), and the deleted UID must be the **single** message in that folder whose Message-ID header exactly equals the draft's persisted Message-ID — IMAP substring search hits are individually header-verified, and zero or multiple exact matches skip cleanup as ambiguous. Any unverifiable fact skips cleanup; a failure is logged and never alters the send result. If sent-state persistence itself fails after transmission, no surface reports durable success (the sweep emits `sent_unrecorded`; the CLI/MCP send errors explicitly), cleanup is skipped, and the owner lease parks the draft as the terminal-recovery **`delivery_uncertain`** state — one atomic statement that also clears `send_after` and the lease. That state is non-editable, non-approvable, non-queueable, never due, and never claimable: no dashboard approval or send can promote it back into a sendable draft. Recovery is an explicit operator reconciliation — verify delivery (Sent folder / recipient), then discard the draft — never approval. If even the park fails, the row simply remains in its `sending` claim; in every combination the transmitted draft is out of the due query and cannot be re-selected and resent. Cleanup identity needs only the exact detected folder + persisted Message-ID (a stored `imap_uid` is neither required nor trusted), and the `imap_draft_deleted` result field reports the **actual** cleanup outcome — never inferred from UID presence or absent local state. Scheduled `send --at` values are parsed to canonical RFC 3339 UTC (`Z`): explicit offsets are honored, and a naive local time that is ambiguous (DST fall-back) or nonexistent (spring-forward gap) is rejected with instructions to supply an offset, never silently relabeled as UTC. The gate mode comes from the build (see **Governor is a build-time feature** above), never from the environment: required mode fails closed on missing Governor, execution error, `review`, or `deny`, and only an explicit Governor `allow` permits SMTP; off mode scores nothing and still refuses a missing or invalid declaration. Governor itself receives only the declared attribute keys plus a content-free justification (surface + draft id); Envelope's own audit payload additionally holds sanitized metadata (subject hash, recipient counts/domains/classes, surface, draft id, attachment counts/sizes/types, reply flag, and the declared attribute keys) — never bodies, attachment bytes, secrets, or full recipient addresses.

### Crash safety: intents, attempts, receipts

Reported by `envelope contract` under `outbound_safety.crash_safety`.

**Intents and idempotency.** Every send request (CLI `send`, MCP `send`/`reply`; immediate, queued, or `--at`) is written as a durable intent row, holding the content and the attachment bytes, before any network work. `draft send` / `send_draft` use the draft id as the operation id. A rerun of the same request answers from that row and sends nothing new:

- With an explicit key (`--idempotency-key` / `idempotency_key`: printable ASCII, up to 200 characters, stored only as a hash, scoped to the account and the requesting agent or local operator), the key names one message for as long as its row exists, including after discard. The same key with different content returns `status: "idempotency_key_conflict"` and sends nothing.
- With no key, an identical request (same account, requester, sender, recipients by role, subject, bodies, threading and attachment bytes) is matched while the earlier intent is unresolved, queued, uncertain, or awaiting review, and for 15 minutes after it was sent. After that an identical request is a new message. Only a key gives the hard guarantee; the fingerprint is a heuristic for crash reruns, and it suppresses a deliberate identical resend inside that window.

A matched request makes no Governor call. A `sent` intent returns its recorded result with `idempotent_replay: true` (exit 0); a queued one returns its `draft_id`; a row that may have been delivered, awaits review, is being sent by another live process, or was discarded returns its `status`, `retryable` and `error.code` (exit 1).

**Attempts.** Each claim of a row is one attempt with its own Message-ID, recorded before the claim commits. The attempt commits `transmitting` after the server answers DATA and before the first body byte; a worker that loses that compare-and-set stops before the body. Until then any failure releases the row (`status: "not_sent"`, `retryable` false only for a permanent 5xx). After it, a 4xx/5xx reply to the body still proves nothing was accepted and releases the row; a lost connection or a missing reply parks it `delivery_uncertain` (`retryable: false`), and nothing re-sends it. The outcome is recorded before QUIT, and each stage has a deadline (10 minutes to DATA, 10 minutes for the body, 10 seconds for QUIT). In the scheduled sweep a permanent refusal parks the row `blocked` with a `send_block` reason instead of retrying it every tick.

**Recovery.** A row left `sending` by a process that died is resolved by the next send, `draft show`, `draft list`, and every sweep tick (and at `serve` start): an attempt that never reached the body returns to `draft` (a scheduled row comes due again), one that had started it becomes `delivery_uncertain`. The sending process holds an operating-system file lock for the whole attempt, so a live sender, however slow or stopped, is never touched, and a reused PID or a clock change cannot fake a dead one. An owner on another host is resolved the same way after a 15-minute lease. Rows claimed by a binary that kept no attempt record are parked after the lease.

**Receipts.** Every send transition writes one `action_log` row, in the same transaction as the state change: `action_type: "send"`, `draft_id` (the operation id), `message_id`, `agent_id`, and `action_status` = the row's status after the transition in `draft show` terms. `action_taken` is `envelope.send_receipt.v1` JSON: `surface`, `attempt_id`, `seq`, `from`, `to`, `phase`, `reason`, `replay`, `retryable`, `recipients` (local only), `recipient_count`, `payload_sha256` (`envelope.payload.v1`, every transmitted field), `semantic_sha256` (`envelope.semantic.v1`: subject, lowercased recipients and normalized text; byte-for-byte the Mailroom bench's payload hash), `key_kind`, and `evidence`. `send_completed` events carry the Message-ID in their `message_id` column. An agent's receipts also emit `agent_action` events, which carry identifiers and a recipient count, never addresses. `actions tail --json` rows include `agent_id`. If an audit write fails before SMTP the send stops with `audit_unavailable`; after acceptance the result stays `sent` and gains `warnings: [{code: "audit_write_failed"}]` with a non-zero exit.

SQLite runs with `synchronous=FULL`, so these records survive power loss as well as a crash.
...
**Relationship facts.** `known_contact`, `frequent_contact`, `cold_email`, `unknown_domain` and `reply_to_thread` come from local evidence inbound mail can't create, judged on exactly the addresses SMTP will deliver to. Outbound history means messages cached from the account's detected Sent folder. A From equal to the account address anywhere else (a spoofed message in INBOX, for example) doesn't count, and MCP `move_message` and `bulk` refuse to move or copy mail into Sent. A contact makes a recipient known only when a person curated it with CLI `contacts add` or `tag`; contacts written over MCP (`add`, `tag`, `untag`) or by `contacts import` stay in the address book and never vouch. `cold_email` fires when any recipient lacks that history, and `unknown_domain` when any recipient's domain is unseen (the account's own domain never is), so one new address among known ones keeps its first-contact signal. `reply_to_thread` needs the `In-Reply-To` to resolve to exactly one cached thread in the account, with every recipient on a Sent-folder message in that thread. Otherwise the send derives as a fresh compose, and a declared `reply_to_thread` is rejected (`conflicts_with_host_observation`), which refuses the request in every build, with or without Governor; resubmit without it to send as new mail. More than 8 recipients, headers SMTP can't parse, or a failed store lookup count as containing a first contact: `cold_email`, and no favorable fact. A lookup that runs out of candidate rows leaves a fact unknown and asserts nothing.
