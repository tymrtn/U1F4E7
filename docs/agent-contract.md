...
At the next scheduled-send sweep, attribute derivation reads the attestation and sets `human_approved=true` on the send context, which declares the `tyler_approved` attribute to Governor on re-scoring. Approval is **an input attribute, not a bypass**: the send still runs the full fail-closed Governor gate, Envelope declares only the honest contextual attributes, and Governor remains free to score a human-approved send as `review` or `deny`. Approval also **supplements, never replaces, a bot's attribution responsibility**: for a bot-originated draft the bot's own factual declaration is still mandatory, and a human approving it adds `tyler_approved` on top rather than waiving the declaration. Only a genuinely human-originated draft (`created_by` = `human:*`) can proceed on the attestation alone.
