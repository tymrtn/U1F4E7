...
- CLI and dashboard Jev mail engine: `engine once`, `engine run`, `engine status`, `engine decisions`, `engine correct`, `engine recover`, `engine digest-queue`, `engine digest`, plus read-only dashboard decisions and revision-guarded corrections
...
`engine once`, `engine run`, and the fresh-call form of `engine recover` accept `--jev-backend openrouter|laya|custom`, which overrides `decisions.provider` in config.json for one run (see docs/decisions.md). The default provider is `openrouter`: `typesafe/jev-1.13` at `https://openrouter.ai/api/alpha/decisions` through the guarded egress client (public addresses only, redirects disabled); the key is read from `decisions.key_env` (default `OPENROUTER_API_KEY`) and never persisted. Each decision sends the normalized sender address/domain, subject, at most 8 KiB of derived plain text, received timestamp, read/unread/junk flags, attachment presence, and bounded indexed sender, interaction, and reply statistics. This is third-party data egress. Credentials, local paths, Message-IDs, recipient lists, attachment bytes, and unsubscribe URLs are excluded.

`--jev-backend laya` is the optional local provider. It uses only the pinned Laya-MLX checkpoint `aac6fef/laya-mlx` at revision `047678560251f28113ee8f5df4be82102c7bf336`, reached at exactly `http://127.0.0.1:8791/decide`. The bundled provider process (`scripts/laya_jev_provider.py`) must already be running with that revision pre-fetched; it resolves the immutable Hub snapshot to an absolute cache path and verifies SHA-256 for every runtime model/configuration/tokenizer file before loading. Envelope never starts a process, discovers an endpoint, honors an endpoint or environment override, sends an API key, downloads weights, or selects a different checkpoint. Redirects and all environment/system proxies are disabled for the Laya client, so `HTTP_PROXY`, `ALL_PROXY`, and macOS proxy configuration cannot intercept loopback email state. Connect timeout is five seconds, Envelope stops waiting after a 30-second request timeout, the request body is capped at 128 KiB, and the response is capped at 64 KiB. Laya receives the identical typed `state` and `questions` payload OpenRouter receives, plus the pinned repository and revision so a provider serving other weights is refused rather than trusted. Laya answers the typed `choice` and `noul` questions natively in a forward pass, so no text or JSON is generated by a language model and email content is never interpolated into an instruction. All message, sender, and history content stays on-device and no content egress occurs. `envelope engine laya-health` reports readiness, the served model and revision, and dtype without sending any message, sender, or history content.

Both backends produce the identical `ValidatedDecision` surface and the identical policy input: independent route, urgency, immediate-notification, reply-requirement, and bulk/subscription results. Envelope first proves the Laya response names the pinned repository and revision, then labels the answer with the durable local identity `aac6fef/laya-mlx:047678560251f28113ee8f5df4be82102c7bf336` — never `typesafe/jev-1.13` — and runs the identical strict shared validator OpenRouter answers pass, including known-option, complete-distribution, probability-range, sum-to-one, and selected-maximum checks. The model check is narrowed per provider: each backend accepts only its own pinned identity, so a local answer can never be stored or displayed as an OpenRouter answer, and an OpenRouter answer can never be accepted as a local one. Code—not Jev—owns policy and side effects. A durable claim records the actual backend and model before the request, so concurrent workers and crash retries cannot spend twice or ambiguously collide across local and remote identities. A wrong model or revision, malformed output, invalid probabilities, an unreachable or unready provider, an oversized response, and timeouts all become a coarse `laya_jev_failed` `review` decision and leave Inbox unchanged. There is no fallback: laya mode never calls OpenRouter. Remote failures retain their existing closed error behavior. Raw prompts/responses are neither logged nor persisted; `engine decisions` lists backend/model, route, urgency, action state, confidence, and closed error code without fetching message content. Claims left `processing` for ten minutes are parked once as `decision_interrupted` for human review without another request.

`engine recover <uid>` immediately releases an abandoned current-epoch processing claim into review without making a Jev call. The `--retry-jev --confirm-new-jev-call` path also requires the selected `--jev-backend` to match the stored backend/model identity. OpenRouter retry remains limited to `openrouter_api_key_missing`, which proves the original request never left Envelope; ambiguous remote network/model failures cannot be retried through this path. Laya retry is limited to the coarse `laya_jev_failed` state, requires the stored `laya` backend and pinned model identity, and remains on-device. Both fetch and reclassify only that exact current-UIDVALIDITY message and never rewind the new-mail watermark.

`engine correct <uid>` and the CSRF-protected dashboard correction endpoint append a revision-guarded local route/urgency override. They never replace the immutable Jev output, retrain the model, or create a mailbox rule. A correction away from junk cancels an unstarted pending move. If a mailbox action already completed, the effective classification changes but mailbox restoration remains explicit and manual.

`envelope engine digest-queue` exposes a bounded, account-filterable list of pending message handles routed to `digest_news` in each mailbox's current UIDVALIDITY epoch; stale numeric UIDs from recreated mailboxes and already consumed items are excluded. It is local and read-only: output contains account, folder, UIDVALIDITY, UID, typed probabilities, urgency, and decision time, but no sender, subject, body, Message-ID, model response, or credential material.

`envelope engine digest` compiles the bounded pending queue. It revalidates UIDVALIDITY at the mailbox edge, opens folders with `EXAMINE`, and fetches only `From`, `Subject`, and `Date` through UID `BODY.PEEK[HEADER.FIELDS ...]` requests. It never fetches a message body or marks a message read. JSON nests sanitized sender/subject/date under `untrusted_content` and attaches the standard `envelope.inbound-trust.v1` marker with `instructions_authoritative=false`; host-owned message handles and typed route metadata remain separate. Recipient fields, Message-ID, flags, message size, provider-spam metadata, credentials, and raw errors are excluded. Account/folder failures are returned as coarse error codes so one unavailable mailbox does not fabricate digest items. `--consume` records only successfully compiled current-epoch items as locally consumed; failed or missing items remain pending, repeated consumption is idempotent, and no mailbox flags or folders change.
...
Urgent external notification is separately opt-in with `--deliver`. When the validated policy says `notify_user_now`, Envelope creates a deterministic, content-free `mail_engine_urgent` event keyed by account/folder/UIDVALIDITY/UID and one idempotent local delivery row per enabled matching route. A retry that finds an existing durable decision re-derives notification intent from the persisted urgency and probability, then repairs a missing event/delivery before advancing the mailbox watermark. Non-terminal `processing` decisions hold the UID range and prevent a later UID from advancing past unfinished work. With `--deliver`, the engine drains due rows through the existing HMAC-signed webhook executor, which records delivery, bounded retries, and dead-letter state; its HTTP client refuses redirects. Without `--deliver`, event and delivery rows remain local and no external notification is attempted, so a later delivering pass can recover them. Configure an event route matching `mail_engine_urgent`; this is notification transport configuration, not a mailbox classification rule.
...
Everything else — CLI, MCP, scheduled, and agent-queued sends, approved or not — runs the full gate with its factual-declaration requirement intact (fail-closed Governor scoring in a `governor` build; the declaration check alone otherwise). Cooldown, Hold/unqueue, Sent-folder proof, and the rest of delivery handling are unchanged for a Human-only Send; the click removes Governor scoring from that one transmission and nothing else.
...
All rule runs (`envelope rule run`, MCP `rules_run`, the dashboard, and `envelope watch --run-rules`) share one executor. Each executed action writes one `action_log` row attributed by `source` (`rule`, `cli`, `mcp`, `reader`, `cairn_action`) and keyed `(event_id, action_type)`, so replaying a rule over the same message reports `status: "already_applied"` and mutates nothing. `add_tag` now persists the tag. Snooze and unsubscribe rules are skipped with a `skipped_rules[].reason` starting `needs_review:` until a human runs `envelope rule enable <name> --acknowledge-batch-actions`; the batch unsubscribe path only performs RFC 8058 one-click HTTPS POSTs (a mailto unsubscribe stays Governor-gated behind `envelope unsubscribe --attr`). A `confirm` rule never acts: it records an `action_offered` event whose actions (`add_tag`, `flag`, `move` never to Trash/Junk) run only after `envelope actions confirm <event_id>`. There is no MCP confirm tool. A rule whose match has an empty `and`/`or` list (it could match every message) is refused on save and skipped when rules run; `rules_preview`, `rules_run`, `rule test` and `rule preview` list it in `skipped_rules` with a reason starting `rule has an empty condition list`.
