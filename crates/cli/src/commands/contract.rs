...
pub fn agent_contract() -> Value {
    json!({
        ...
        "outbound_safety": {
            ...
            "governor_gate": {
                "smtp_mode": GovernorConfig::smtp().mode.as_str(),
                "operator_configuration": "The gate mode is fixed when Envelope is compiled, by the `governor` Cargo feature, and smtp_mode reports it for this binary. Built with the feature (`cargo build --release --bin envelope --features governor` or `cargo install --path crates/cli --features governor`), SMTP-capable CLI, MCP, scheduled-send, and dashboard processes always invoke the trusted absolute Governor executable in required mode, and if that executable is unavailable, SMTP fails closed. Built without it (the default, including Homebrew and the release archives), smtp_mode is `off`: SMTP sends are not scored by Governor and no Governor executable is invoked, while the attribution precondition and the rest of the send policy still apply. Caller-controlled environment variables, CLI flags, and config keys cannot select a mode or a different executable. Diagnostic testing may construct an explicit in-memory gate configuration only when it is not wired to SMTP.",
                "behavior": "When built with the `governor` feature, before any real SMTP send (immediate bypass, scheduled-send sweep, and the `mailto:` compliance unsubscribe), the actual Governor decision engine is consulted using blind attribution: Envelope declares the contextual attribute keys the send exhibits and Governor opaquely scores/routes them against the 'envelope' catalog (allow/review/deny). Envelope never reproduces Governor's weights or thresholds. The scheduled-send sweep re-derives the final host facts from the persisted draft AND loads the declaration the bot validated at queue time, resolves declared ∪ derived, and calls Governor only when attribution is valid; a bot-originated draft with no valid current declaration fails closed there even when the derived set is rich (host facts never substitute). Durable review/deny verdicts park the draft as pending_review (no retry storm) while transient gate failures leave it queued. Missing/error/deny/review all block SMTP; only an explicit allow permits it. Without the feature the same attribution checks run at the same points and the send proceeds unscored, recording mode off and decision disabled. The existing dashboard Human-only Send action remains a separately attested human boundary and is not an agent/configuration bypass.",
                ...
                "attribution": {
                    ...
                    "catalog": "envelope",
                    "rule": "Every bot-originated governed send MUST contain at least one factual declared attribute; host-derived facts never substitute. The attribution precondition fails closed for all SMTP-capable Envelope processes, in every build and gate mode; no caller-controlled environment setting can waive it. Human approval SUPPLEMENTS a bot send (it adds the tyler_approved attestation to the derived set) but never erases the bot's declaration responsibility.",
...
mod tests {
    ...
    fn contract_advertises_cooldown_and_governor() {
        ...
        assert_eq!(safety["governor_gate"]["smtp_mode"], json!("off"));
