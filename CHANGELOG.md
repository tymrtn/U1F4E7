...
- **The first `envelope draft create` on a new data directory works.** It failed with "could not
  append mail to mailbox" on servers that list the Drafts folder before other folders (seen on
  Dovecot 2.4.5), and worked on the second try once the folder was cached. The SPECIAL-USE
  Drafts lookup stopped reading the folder list at the Drafts line, so the APPEND that followed
  read a leftover list line where it expected the server's `+` go-ahead. The lookup now reads
  the whole reply. Reading one message, downloading an attachment, and fetching
...
- `envelope rule create` with no `--match-*` flag is refused. It used to save a rule with an
  empty condition list, and an empty list matched every message, so a delete rule would have
  deleted the whole folder.
- A match with an empty `and`/`or` list anywhere in it is refused on save (dashboard HTTP 400,
...
- **Store:** the CLI and dashboard open a shared database the isolated V2 line has advanced to
  schema version 21 (one nullable column, `agent_policies.allowed_addresses`, for agent address
  scope). The known-compatible ceiling was 20, so a V1 build refused such a database with
  `database schema version 21 is newer than this envelope build supports` and neither the CLI
  nor the managed dashboard could start. As with 17–20, the database is opened exactly as
  found: no migrations run, no `user_version` write, no V2 table or column touched (V1 reads
  `agent_policies` columns by name). Versions past 21 still fail closed naming the found and max
  supported versions.
...
- **Store:** the CLI and dashboard open a shared database the isolated V2 line has advanced to
  schema version 20 (one additive table and its index: `link_redirects` for reader
  instrumentation, or `calibration_verdicts` from the governor calibration branch). The
  known-compatible ceiling was 19, so a V1 build refused such a database with
  `database schema version 20 is newer than this envelope build supports` and neither the CLI
  nor the managed dashboard could start. As with 17–19, the database is opened exactly as
  found: no migrations run, no `user_version` write, no V2 table touched. Versions past 20
  still fail closed naming the found and max supported versions.
...
- **Evidence manifests:** exports now preserve a configured account username
  that is already a complete email address. Previously an account configured as
  `name@example.com` on `example.com` was recorded as
  `name@example.com@example.com`. Local-part usernames still receive their
  configured domain, and the independent IMAP login field remains unchanged.
  Existing evidence bundles are deliberately not rewritten.
